...
| 核心策略 | 基础格子数 | 拆解逻辑 | 适用场景 |
|----------|----------|----------|----------|
| `[单镜]` | 1格 | 直接选择最具表现力的单一画面 | 对话、静态情感 |
| `[动势组]` | 2格 | 分为起幅（动作开始）+ 落幅（动作结束） | 追逐、移动、快速动作 |
| `[蒙太奇组]` | 3格 | 分为Setup（起势）→Action（动作）→Resolution（结果） | 时间压缩、情绪渲染、空间跳跃 |
| `[长镜头]` | 1-3格 | 选取关键姿势，通过镜头运动标注完整轨迹 | 连续动作、复杂调度 |
...
其中：
- 基础格子数 = 根据核心策略确定（见上表）
- 额外格子数 = max(0, round(复杂度评分 - 2.5))

限制条件：
- 调整后格子数 ≤ 4（单板最大格子数）
- 调整后格子数 ≥ 基础格子数
- 蒙太奇组最多3格，单镜最多1格，动势组最多2格
```
