...
> 详细规范请参考 `.claude/common/*.md` 目录下的具体实现文档。
>
> **核心改进（V3.0）**：
...
> - ✅ **V4.0新增**：动态分镜引擎（.claude/common/dynamic-breakdown-engine.md）
> - ✅ **V4.0新增**：四宫格裂变协议（支持跨板连续性）
>
> **文件说明**：
> - **AGENTS.md**: 主配置 - 定义整体架构、流程、协议（本文档）
> - **.claude/workflows/**: 工作流程文档 - 各命令的详细工作流程说明（新增）
> - **.claude/common/*.md**: 详细规范 - 作为主配置的扩展说明（各模块具体规范）
...
## 核心信息
    ...
    ### 指令路由表（Resume 机制）
        ...
        | ------------------- | ----------------- | --------------------- | --------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
        | `/AINIEO`           | 制片人            | 初始化欢迎 + 配置向导 | -                     | 无          | -                                                                                                                                                           |
        | `/breakdown [集数]` | storyboard-artist | 节拍拆解              | film-storyboard-skill | Resume/首次 | [script/*-{集数}.md] + [scene-breakdown-{集数}.json] + [.agent-state.json: visual_style, target_medium, aspect_ratio, narrative_structure, genre, enable_*] |
        | `/beatboard [集数]` | storyboard-artist | 九宫格关键帧选集      | film-storyboard-skill | Resume/首次 | [scene-breakdown-{集数}.json, beat-breakdown-{集数}.md]（包含新字段：核心策略、辅助信息、关键信息点、复杂度评分、建议格子数、拆解建议）                     |
          输出: outputs/beat-board-prompt-{集数}-board*.md（每格=1个关键帧）
          规则: 不包含动作拆分，动作拆分由 /sequence 阶段完成
          单帧约束: 禁止"一格多画面"，使用戏剧张力算法选择最强帧
          JSON记录: alternative_frames 存储未选择帧（方案B：类型+reason） |
         | `/sequence [集数]` | storyboard-artist | 动态分镜板提示词（支持可变格子数1-4格） | film-storyboard-skill | Resume/首次 | [scene-breakdown-{集数}.json, beat-board-full-list-{集数}.json, beat-board-prompt-{集数}-board*.md]（读取节拍拆解中的：核心策略[含策略标签]、复杂度评分、建议格子数） |
         | `/motion [集数]` | animator | 动态提示词 | animator-skill | Resume/首次 | [scene-breakdown-{集数}.json, sequence-board-data-{集数}.json, sequence-board-prompt-{集数}.md, ...] |
         | `/review breakdown [集数]` | director | 审核节拍拆解 | storyboard-review-skill | Resume/首次 | [scene-breakdown-{集数}.json, beat-breakdown-{集数}.md, ...] |
         | `/review beatboard [集数]` | director | 审核九宫格 | storyboard-review-skill | Resume/首次 | [scene-breakdown-{集数}.json, beat-board-full-list-{集数}.json, beat-board-prompt-{集数}.md, ...] |
         | `/review sequence [集数]` | director | 审核四宫格 | storyboard-review-skill | Resume/首次 | [scene-breakdown-{集数}.json, sequence-board-data-{集数}.json, sequence-board-prompt-{集数}.md, ...] |
         | `/review motion [集数]` | director | 审核动态 | storyboard-review-skill | Resume/首次 | [scene-breakdown-{集数}.json, motion-prompt-data-{集数}.json, motion-prompt-{集数}.md, ...] |
         | `/status` | 制片人 | 状态检测 | - | 无 | - |
         | `/snapshot` | 制片人 | 快照管理 | - | 无 | - |
         | `/logs [子命令]` | 制片人 | 日志查询 | - | 无 | - |
         | `/creative [指令]` | storyboard-artist | 添加创意 | film-storyboard-skill | Resume/首次 | [对应主产物] |

     **日志查询命令**（制片人直接处理，不调用 subagent）：
     - `/logs` - 显示最近3次执行摘要
     - `/logs last` - 显示最近1次执行详情
     - `/logs <exec-id>` - 显示特定执行详情
     - `/logs files` - 文件操作统计
     - `/logs deps <file>` - 文件依赖图
     - `/logs timeline` - 执行时间线
     - `/logs modules` - 模块使用统计
     - `/logs debug` - 调试信息
     - `/logs search <关键词>` - 搜索日志记录
    ...
    **调用方式说明**：
    ...
    ### 知识库调用：
       The real-time project file structure is maintained in PROJECT_MAP.md. Always read that file to locate specific scripts or assets. (知识库：实时的项目文件结构维护在 PROJECT_MAP.md 中。在查找具体脚本或资产时，务必读取该文件。)

    ### 🚨 File Creation Protocol (文件创建强制协议)
        为了确保 `PROJECT_MAP.md` 能自动抓取并更新项目地图，**所有 Agent 在创建新文件时必须遵守以下规则**：
        1.  **Markdown 文件 (.md)**：
            文件的**前三行内**必须包含一行元数据描述，严格使用以下格式：
            > `> **Description:** <这里写该文件的简短中文用途>`
...
## 强制流程检查点（方案B：含 Resume 机制）
    ...
    **每个阶段必须完成以下5个步骤，缺一不可**：
    ...
    | 步骤 | 检查点                              | 执行者                     | 产出                                      | 阻塞条件               | Resume 相关           | 日志记录方式 |
    | ---- | ----------------------------------- | -------------------------- | ----------------------------------------- | ---------------------- | --------------------- | ------------ |
    | 1    | 生成主产物 + SKILL自检 + Resume调用 | storyboard-artist/animator | 主产物文件 + agentId                      | SKILL自检失败          | ✅ 首次/Resume调用     | ✅ SKILL内部LOG标记（自动记录） |
    | 2    | Director审核 + Resume调用           | director                   | xxx-review.md + 审核状态                  | 审核不通过 → 返回步骤1 | ✅ 首次/Resume调用     | ✅审核SKILL LOG标记（自动记录） |
    | 3    | 用户确认（CP1-CP5）                 | 制片人                     | 用户决策                                  | 用户未确认             | ❌                     | ✅ 用户操作日志（通过question工具记录） |
    | 4    | 创建快照                            | 制片人                     | .strategic-snapshots/ + .agent-state.json | 快照创建失败           | ❌ 包含状态文件        | ✅ 快照日志（自动记录snapshot创建） |
    | 5    | 更新phase状态 + subagent状态        | 制片人                     | .agent-state.json                         | 状态未更新             | ✅ 更新agentId和时间戳 | ✅ 状态日志（自动记录.agent-state.json更新） |
...
## 总体规则
    ...
    - 审核任务由 director 执行（调用 storyboard-review-skill，必须传递上游产物）
     - **每个阶段完成后必须执行以下5个步骤**：
       ...
       4. 用户显式确认后 - 创建快照（包含 .agent-state.json）
       5. 用户显式确认后 - 更新 phase 状态 + subagent 状态
    ...
    - 地图维护协议：
        感知触发：凡是涉及 write_file（写入）、rename（重命名）或 delete（删除）操作后，AI 必须立即询问：“检测到文件变更，是否运行 python3 generate_map.py 更新地图？”

        执行指令：用户输入 /sync 时，AI 必须通过 OpenCode 的终端或代码执行工具运行 generate_map.py。

        结果验证：同步后，AI 需简要列出 PROJECT_MAP.md 中发生变动的路径。
     - 始终使用**中文**进行交流
...
**1. LOG标记识别**
- SKING自动识别 `<!-- LOG:STEP -->` 标记
- 从标记中提取：文件读取、模块调用、算法使用、输出内容、备注
- 自动计算步骤耗时并记录时间戳

**2. 文件行数统计**
- 智能混合格式：
  - `filename` → 自动计算总行数并显示 `(行1-总行数)`
  - `filename:1-50` → 显示 `(行1-50)`
  - `filename:1-50,60-120` → 显示 `(行1-50, 60-120)`
- 支持分段读取，精确追踪实际使用范围

**3. 日志文件管理**
- 日志文件：`.claude/.execution-log.json`
- 追加模式：每次执行生成新的execution_id
- 自动轮转：保留最近50条执行记录，自动清理旧日志

### 查询命令系统

| 命令 | 功能 | 示例输出 |
|------|------|----------|
| `/logs` | 显示最近3次执行摘要 | exec-003, exec-002, exec-001 |
| `/logs <exec-id>` | 显示特定执行详情 | 步骤耗时、文件读取、模块调用 |
| `/logs last` | 显示最近1次执行详情 | 完整日志 |
| `/logs files` | 显示文件操作统计 | 读取频率、写入文件 |
| `/logs deps <file>` | 查看文件依赖图 | 依赖/被依赖关系 |
| `/logs timeline` | 显示执行时间线 | 可视化时间轴 |
| `/logs modules` | 显示模块使用统计 | 使用频率排名 |
| `/logs debug` | 显示调试信息 | 最近错误、警告 |
| `/logs search <keyword>` | 搜索日志记录 | 按关键词过滤 |

### 5步流程中的日志记录点

| 步骤 | 检查点 | 执行者 | 日志记录方式 |
|------|--------|--------|-------------|
| 1 | 生成主产物 + SKILL自检 | storyboard-artist/animator | ✅ SKILL内部LOG标记（自动记录） |
| 2 | Director审核 | director | ✅审核SKILL LOG标记（自动记录） |
| 3 | 用户确认（CP1-CP5） | 制片人 | ✅ 用户操作日志（通过question工具记录） |
| 4 | 创建快照 | 制片人 | ✅ 快照日志（自动记录snapshot创建） |
| 5 | 更新phase状态 | 制片人 | ✅ 状态日志（自动记录.agent-state.json更新） |
...
[步骤2] 执行节拍拆解 (3200ms) ⚠️ 最耗时
  📖 读取:
    - .claude/common/beat-analyzer.md (行1-1152)
  🧩 模块: creative-engines-integration
  🔬 算法: beat_analysis, drama_weight_v4.1, type_correction
  📝 输出: shot_list, strategy_tags, complex_scores
  💬 备注: 完整读取beat-analyzer包含戏剧权重映射和类型修正表

[步骤3] 创意引擎调用 (2800ms)
  📖 读取: .claude/skills/film-storyboard-skill/creative-engines-integration/01-engine-specs.md (行1-full)
  ⚙️ 引擎: montage_logic, film_association
  📝 输出: engine_analysis
```
...
[14:30:28.581] ✓ 步骤3: 创意引擎分析 (2800ms)
[14:30:31.181] ✓ 步骤4: 质量检查 (2600ms)
[14:30:37.200] ✓ 完成 (15200ms)
...
1. 节拍拆解 (步骤2)
   ...
   - 最慢: 4.5s (exec-007)
   - 最快: 2.8s (exec-003)
   - 建议: beat-analyzer.md(1152行)较大，考虑优化读取

2. 关键帧选择 (步骤3)
   - 平均耗时: 2.4s
   - 最慢: 3.1s (exec-005)
   - 建议: keyframe-selector.md(385行)性能良好
```
...
script/猪打镇关西_v2.0.md (行1-245)
  └─> outputs/beat-breakdown-ep01.md
  └─> outputs/scene-breakdown-ep01.json
       └─> outputs/beat-board-prompt-ep01-board1.md
       └─> outputs/beat-board-prompt-ep01-board2.md
            └─> outputs/sequence-board-prompt-ep01-board1.md
                 └─> outputs/motion-prompt-ep01.md
```

### Resume模式下的日志连续性

当使用Resume机制调用子Agent时：
...
生成 exec-001，记录完整日志
  ↓
Resume调用: /beatboard ep01 (继续使用storyboard-artist)
  ↓
生成 exec-002，延续上下文
  ↓
日志自动关联:
...
```json
{
  "step_name": "步骤4: 执行连贯性检查",
  "status": "failed",
  "error": {
    "type": "coherence_check_failed",
    "message": "角色一致性检查失败",
    "files_read": ["coherence-checker.md (行1-full)"],
    "algorithm_used": "continuity_check_v4.0",
    "suggestions": [
      "检查角色设定是否完整",
      "验证跨板角色切换逻辑"
    ]
