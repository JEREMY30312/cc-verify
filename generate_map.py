"""
> **Description:** 增强版项目地图生成器 V2.3 - 支持一键安装/卸载/升级
...
try:
    from modules.llm_task_generator import (
        detect_file_changes,
        write_tasks_json,
        wait_for_ai_completion,
    )
    from modules.description_cache import (
        load_cache,
        save_cache,
        clean_cache,
        get_description,
...
def generate_ai_index(
    files_info: List[FileInfo], git_changes: List[GitChange]
) -> Dict[str, Any]:
    """生成AI优化索引"""
...
def generate_project_fingerprint(
    ...
    # 关键目录
    key_dirs: Dict[str, Dict[str, Any]] = defaultdict(lambda: {"count": 0, "files": []})
    for f in files_info:
                    ...
                    [
                        (
                            ...
                            sum(
                                1
                                for x in files_info
                                if x.name.endswith(Path(f.name).suffix)
                            ),
                        )
                        for f in files_info
...
def main():
    ...
    # 3. LLM智能描述生成（如果启用）
    if DESC_MODE == "llm" and LLM_MODULES_AVAILABLE:
        print("🤖 检查文件描述...")
        ...
        if tasks:
            ...
            # 读取AI生成的描述
            ai_cache = load_cache(DESC_CACHE_FILE)
            ai_descriptions = {
                k: v.get("description", "")
            ...
            # 合并到文件信息
            for f in files_info:
                desc = get_description(ai_cache, f.path)
                if desc:
                    f.description = desc
            ...
            cleaned_cache = clean_cache(ai_cache, current_files)
            save_cache(cleaned_cache, DESC_CACHE_FILE)
        else:
            print("   ✅ 所有文件描述已是最新")
    else:
...
python3 generate_map.py --silent
"""
...
def install(repo="JEREMY30312/cc-verify", branch="main"):
    ...
    if os.path.isdir(".git"):
        ...
        try:
            ...
            if choice.lower() == "y":
                subprocess.run(
                    ["python3", "generate_map.py", "--install-hooks"], check=True
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="项目地图生成器 V2.3")
