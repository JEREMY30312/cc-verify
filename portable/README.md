...
> **Description:** 单文件项目地图生成器，支持Git集成、LLM智能描述、学习机制、YAML描述提取
...
| `description.mode` | 字符串 | "llm" | 描述生成模式：llm=LLM智能生成，traditional=传统YAML提取 |
| `description.max_length` | 数字 | 20 | 描述最大长度 |
| `description.cache_file` | 字符串 | ".file-descriptions.json" | 描述缓存文件路径 |
| `description.tasks_file` | 字符串 | ".llm-tasks.json" | 任务清单文件路径 |
| `description.timeout_seconds` | 数字 | 60 | 等待AI完成描述的超时时间（秒） |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 2.2 | 2026-04-09 | LLM智能描述生成、寄生式架构、跨平台兼容 |
...
使用AI自动生成文件描述，无需手动编写YAML front matter。

**工作流程**：
1. 扫描项目，检测需要生成描述的文件
2. 生成任务清单 `.llm-tasks.json`
3. 等待AI平台处理（轮询检测，超时60秒）
4. 读取AI生成的描述 `.file-descriptions.json`
5. 生成项目地图
...
使用YAML front matter或Markdown引用块提取描述。
